...
<tim chngno="0" revno="0" chap-toc="no">\n'''
        ...
        with open(self.save_path + '/' + self.sys_acronym + ' ' + self.manual_type +
                ' WIP/{:05d}-TS_MASTER_INDEX_START.txt'.format(cfg.prefix_file), 'w', encoding='UTF-8') as _f:
            _f.write(tmp)
        cfg.prefix_file += 10

    def tsindxwp(self):
        """Function to create a Troubleshooting Index WP."""
        tmp = '<tsindxwp chngno="0" wpno="T00000-' + self.sys_number + '">\n'
        tmp += '\t<wpidinfo>\n'
    ...
    <tsindx.symptom>
        ...
        with open(self.save_path + '/' + self.sys_acronym + ' ' + self.manual_type +
                ' WIP/{:05d}-T00000-TSMasterIndex.txt'.format(cfg.prefix_file), 'w', encoding='UTF-8') as _f:
            _f.write(tmp)
        cfg.prefix_file += 10
    ...
    def end(self):
        ...
        tmp += '</tim>'
        with open(self.save_path + '/' + self.sys_acronym + ' ' + self.manual_type +
                ' WIP/{:05d}-TS_MASTER_INDEX_END.txt'.format(cfg.prefix_file), 'w', encoding='UTF-8') as _f:
            _f.write(tmp)
        cfg.prefix_file += 1       